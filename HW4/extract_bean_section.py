...
IMG.append(Image("smooth + threshold", binaryBeans, PLOC[1])) #IMG[1]

# connected component segmentation
segment, seg_count = ndi.label(binaryBeans)
bean_properties = regionprops(segment)
IMG.append(Image("connected segments", segment, PLOC[2])) #IMG[2]
# plot segments and their labels
ezplot(IMG[2].ezplot_info(), ax, colors=plt.cm.Set1_r)
bean_coords = []
for i in bean_properties:
    bean_coords.append(np.array(i.centroid, dtype=int))
bean_coords = np.array(bean_coords)
ezplot( (bean_coords, PLOC[2], "points"), ax, plotType='pointOverlay', colors='k.')
ax[PLOC[2][0]-1][PLOC[2][1]-1].set_axis_off()
for n in range(bean_coords.shape[0]):
    ax[PLOC[2,0]-1][PLOC[2,1]-1].annotate(n+1, xy=(bean_coords[n][1], bean_coords[n][0]),
                                          textcoords='data', horizontalalignment='left',
                                          verticalalignment='top', color='black')

# extract boundary of largest segment
...
print(f"Biggest bean-group award: #{biggestBean[0]} at {biggestBean[1]} pixels!")
bigBean = bean_properties[biggestBean[0]-1].image
bigBean = skyler.expand_canvas(bigBean, 20)
RGBean = (ski.color.gray2rgb(bigBean*255)).astype(np.uint8)
iio.imwrite('biggest_segment.png', RGBean, extension='.png')
...
IMG.append(Image("boundary of #6", -edgeBean, PLOC[3])) #IMG[3]

# compute gaussian derivatives (x', y', x", and y")
imgt = bigBean * 255
dx, dy, d2x, d2y = skyler.compute_derivatives(imgt, sigma=2)
IMG.append(Image("dx", dx, PLOC[4])) #IMG[4]
IMG.append(Image("dy", dy, PLOC[5])) #IMG[5]
IMG.append(Image("d2x", d2x, PLOC[6])) #IMG[6]
IMG.append(Image("d2y", d2y, PLOC[7])) #IMG[7]
K = skyler.compute_curvature(imgt, dx, dy, d2x, d2y)*255
IMG.append(Image("curvature of #6", K, PLOC[4])) #IMG[4]
